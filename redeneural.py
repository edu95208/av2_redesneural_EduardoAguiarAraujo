...
def somatorio(entradas, peso):
    print(f'Peso do Somatório que séra selecionado = {peso}')
    constante = 0
    valor_de_somatorio = 0
    for e in entradas:
        valor_de_somatorio += e['valor'] * e['pesos'][peso]
    return round(valor_de_somatorio + constante, 2)
...
def run():
    ...
    print('\n=-=-=-=-=-=-=-= inicio =-=-=-=-=-=-=-=\n')
    print(f'Quantidade em entradas: {qtd_entradas}\nQuantidade de pesos por entrada: {qtd_pesos}\n')
    ...
    print(f'Valor de Função dos Custo: {custos}')
